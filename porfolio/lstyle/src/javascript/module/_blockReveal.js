(function($) {
    ...
    var MODULE = {
        name: 'blockReveal',
        title: 'BlockReveal 효과',
        desc: '사각 마스크 모션 생성 - 작성 중',
        version: '0.1'
    };
    ...
    $.fn.blockReveal = function(_options){
        ...
        return this.each(function(i){
            ...
            var _toggle = function(_el, _bool){ // on 일때
                ...
                if (_bool === undefined) { // toggle
                    _bool = (_el.hasClass('is-in-block-reveal')) ? false : true;
                }
                ...
                var _core = function(){
                    _el.toggleClass('is-in-block-reveal', _bool); // flag
            ...
            var _init = function(_el, _opts){ // 초기화
                ...
                var _setHandler = function(_el){ // 핸들러 부여
                    _el.off('.blockReveal').on({
                        'in.blockReveal': function(){
                            clearTimeout(_timerAutoPlay);
                            _toggle(_el, true); // on
                        },
                        'out.blockReveal': function(){
                            clearTimeout(_timerAutoPlay);
                            _toggle(_el, false);
                        }
                    });
                ...
                for (var i = 0; i < _length; i++) {
                    ...
                    if ($this.hasClass('init-block-reveal')) return false;
                    ...
                    $this.prepend(_html);
                    $this.addClass('init-block-reveal');
